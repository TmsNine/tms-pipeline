...
Per-task working files (the 8 durable pipeline artifacts: `00_ticket` → `01_research` → `02_design` →
`03_plan` → `04_implementation` → `04b_review` → `05_test_report` → `06_review_gate`) live next to your code under `<task-folder>/`, not here. This vault holds the cross-task
