...
> Давай сделаем ТЗ продукта **<продукт>**. Идея в **<суть>**; цель — **<результат>**.
> Задавай мне вопросы **по одному**, каждый с 2–3 конкретными вариантами ответа, и **помечай
> рекомендованный вариант**. Я буду отвечать; когда вопросов не останется — сделай файл `PRD.md`.
> Затем разложи все решения из моих ответов по папкам шаблона документации tms-pipeline
...
- Настоящий **PRD** и остальную базу документации, заполненную вашими решениями, а не догадками агента.
...
Когда появятся база документации и хотя бы одна задача в бэклоге — запускайте `npx tms-pipeline` и
начинайте конвейер как обычно (ниже).
...
# 2b) Codex — читает AGENTS.md нативно; поставьте скиллы/агентов для Codex
#     (см. docs/02-configuration.ru.md#codex)
```
