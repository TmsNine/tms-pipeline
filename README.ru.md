...
| 04 Реализация | `/tms-implement` | Пишет код волна за волной: M остаётся у lead, E получает bounded evidence/test help, R/C — отдельного Developer и proving-роли с readiness gate не ниже 8/10 до 04b. |
| 04b Цикл ревью | `/tms-loop-review` | Для R/C начинает с двух независимых ракурсов, чинит объединённый пакет и делает fresh final review; после трёх неуспешных попыток останавливается на перепланирование. |
...
2. **Риск-профили, которые решают, где покупать качество.** Каждая волна реализации получает профиль
   ...
   proving-role режим. M/E остаются дешёвыми, а R/C получают реальное разделение code owner, Validator,
   Architect/Security и stage-04 Reviewer. В 04b передаётся уже проверенная реализация, а не сырой diff.
...
agents/        5 proving-ролей Claude (разработчик, тестировщик, архитектор, безопасник, ревьюер)
codex-agents/  9 TOML-ролей Codex (включая Developer, Architect, Security и stage-04 wave reviewer)
