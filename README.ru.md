...
Работа разбита на восемь шагов. Каждый шаг вы запускаете отдельной командой агенту (такая команда здесь
...
- **Что это.** Процесс из восьми шагов, который ведёт **одну уже сформулированную** задачу от тикета до
...
- **Основная работа — думание на бумаге.** Семь из восьми шагов дают текстовый документ (`.md`). Код
  появляется только на одном шаге — 04 (реализация). Сначала всё продумывается в тексте, и только потом
  пишется код.
...
- **Посмотреть вживую.** [Полный пример прогона задачи через все 8 шагов →](templates/example-task/ACME-101/)
...
Есть восемь основных шагов от 00 до 06, плюс цикл ревью 04b между реализацией и отчётом о тестах. Шаг
02b (аудит пробелов) стоит между 02 и 03.
...
| 03 План | `/tms-plan` | Разбивает работу на небольшие законченные куски — «волны»; для каждой задаёт профиль эскорта (сколько проверяющих агентов звать). |
| 04 Реализация | `/tms-implement` | mob — команда агентов-ролей: главный агент раздаёт работу подчинённым и сам код не пишет; работа идёт волнами, между ними контрольные точки (гейты). |
| 04b Цикл ревью | `/tms-loop-review` | Независимо ревьюит diff реализации, чинит подтверждённые находки и фиксирует результат цикла. |
...
1. **Аудит дизайна с оценкой тяжести дыр.** До написания кода другой агент намеренно ищет в дизайне дыры —
   ...
   тексте, а не в коде.
2. **Профили эскорта — способ не звать лишних проверяющих.** Профиль эскорта — это сколько проверяющих
   агентов подключить к волне. Каждая волна реализации получает профиль **A** (минимум: разработчик,
   тестировщик, ревьюер), **B** (плюс архитектор) или **C** (плюс безопасник). Полный эскорт — только для
   участков кода, где легко сломать важное: вход и авторизация, разделение данных между клиентами, оплата,
   личные данные пользователей. Какие именно участки рискованные — задаёте *вы*. Тяжёлое ревью включается
   только там, где оно окупается; «включить всё на всякий случай» прямо не приветствуется.
...
- закройте метки `<<TODO: ...>>` (это намеренные заглушки в файле настроек, которые надо заполнить под ваш
  проект) — в первую очередь **`PROFILE_C_TRIGGERS`** (на каких участках кода нужен полный набор
  проверяющих агентов с безопасником) и вашу модель доступа: есть ли в системе отдельные клиенты, чьи
  данные нельзя смешивать (арендаторы), и как определяется, кто перед системой;
...
Всё остальное (расширение до восьми шагов, аудит дизайна с оценкой тяжести дыр, профили эскорта, фиксация
follow-up и ручных действий перед запуском, а также сама упаковка) — оригинальная разработка этого проекта.

## Лицензия

[Apache-2.0](LICENSE). Свободно использовать и адаптировать. Относитесь к процессу как к живому — меняйте
названия шагов, условия для полного эскорта и промпты под культуру вашей команды; важен принцип:
