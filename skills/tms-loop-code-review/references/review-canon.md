...
- Read the repository `AGENTS.md` / `CLAUDE.md` and resolve the exact active change set before review.
...
- Every scoring pass uses a fresh read-only `Agent` subagent with no parent reasoning, suspected bugs, prior findings, prior scores, fix explanations, round number, remaining budget, or acceptance target.
- Use a self-contained prompt and the runtime's clean subagent context. The reviewer must inspect the repository state and diff itself.
- Use a balanced strong tier for ordinary M/E review and the strongest available reasoning tier for R/C, auth/tenant scope, payments, PII, migrations, queues, lifecycle, data integrity, or reviewer disagreement.
- Never use Fast mode for a scoring reviewer.
...
- The lead verifies every finding against code, contracts, tests and runtime evidence before acting.
...
A high score never overrides an unresolved correctness, security, privacy, money or data-integrity issue. Any implementation/test/SQL/contract/config change after the final reviewer invalidates acceptance immediately.
