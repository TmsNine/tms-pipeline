...
the skill. Use Claude Code's `Agent` tool for the mandatory completeness critic.
Fall back to a local pass only when the Agent tool is genuinely unavailable or
the user explicitly opts out, and record the limitation in the stage artifact
and final summary.
...
5. **Completeness critic.** Spawn one fresh `general-purpose` critic through the
   `Agent` tool with a self-contained prompt. Give
   ...
   Use `sonnet` for ordinary judgement and `opus` for Class A/B security/
   privacy/payment/data-integrity. Record requested and actual/unknown runtime
   model. Re-sweep an under-covered zone; do not paper over the gap in
...
bundles. Recommend `tms-audit-backlog` only after the user approves the bundles
