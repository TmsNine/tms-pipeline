...
e.g. `docs/<TICKET-ID>/`) — one folder per task. Fill the 8 files in order:
...
5. `03_delivery_plan.md` — work split into waves, each with an escort profile.
6. `04_implementation.md` — wave-by-wave execution log + deviations.
7. `05_test_report.md` — primary (user-visible) + secondary signals.
8. `06_review_gate.md` — verify vs the design contract; go / conditional_go / no-go.
...
Only these 8 files are durable artifacts. Supplemental durable material (runbooks, SQL playbooks,
