...
description: "Codebase-audit stage 2 — sweep ONE immutable owner/seam zone for code defects, architecture debt, technical debt, false-green tests, and control-plane drift using a bounded fingerprinted code scout when ownership is unclear, followed by an adversarial finder↔skeptic duel. Record only findings that survive refutation. Run once per zone, each in a fresh context window; no arg = next pending zone from the manifest. Second of the tms-9x-audit-* pipeline. Use when the user invokes tms-91-audit-sweep."
allowed-tools:
  - Read
  - Write
  - Bash
...
If `spawn_agent` is not visible, use tool discovery for multi-agent tools. Spawn
the scout, finder, and skeptic with the exposed clean-context mechanism
(`fork_turns: "none"` in the current schema). The skeptic must NOT see the
finder's reasoning as your endorsement, only bare claims to refute.

Model tiers:

- Code scout: exact `gpt-5.6-terra` medium for an eligible broad/unknown-owner
  zone; evidence map only, no findings or severity. If unavailable, use bounded
  local mapping and record the limitation; do not silently substitute a model.
...
2. **Pick the zone.** If `$1` names a zone, use it. If `$1` is empty or `next`,
   ...
   the sweep is complete and to run `tms-92-audit-triage`; stop.
...
5. **Run one bounded code scout only when eligible.** Use it when the zone is
   ...
   fresh `gpt-5.6-terra` scout with no parent conversation, explicit read-only
...
`tms-91-audit-sweep` again for the next zone in a fresh window, or
`tms-92-audit-triage` once all zones are done. One zone per window — do not chain
