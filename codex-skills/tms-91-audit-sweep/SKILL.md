...
- Code-map explorer: `gpt-5.3-codex-spark` high when the zone is broad and the finder needs a compact map first. It returns only `path:line`, symbol/route/component name, snippet/signature, and why it matters; it does not classify final severity.
- Finder: `gpt-5.4` high for ordinary code zones; `gpt-5.5` high for auth/RLS/payments/PII/migrations/queues/lifecycle zones; `gpt-5.3-codex-spark` medium only for docs/copy/dead-code seed sweeps where tool output is the main evidence.
