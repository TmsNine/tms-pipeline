---
name: tms-90-audit-scope
description: "Codebase-audit stage 1 — establish an immutable full-or-delta audit snapshot, inventory code and control-plane surfaces, cut technical and architectural debt into context-sized owner/seam zones, and write the audit folder + manifest. First of the tms-9x-audit-* pipeline (scope → sweep → triage → backlog). Use when the user invokes tms-90-audit-scope or wants to start or resume a codebase audit of unfinished work, defects, architecture debt, technical debt, false-green tests, or release/document drift."
allowed-tools:
  - Read
  - Write
  - Bash
...
```
tms-90-audit-scope   → 00_scope.md + manifest.md   (this skill)
tms-91-audit-sweep   → areas/<zone>.md             (run once per zone, fresh window)
tms-92-audit-triage  → 01_triage.md                (dedup + classify + proposed bundles, then STOP for review)
tms-93-audit-backlog → 02_backlog.md + backlog rows (after approval)
```
...
`can_start_sweeps: yes|no`. If yes, the next step is `tms-91-audit-sweep` once per
