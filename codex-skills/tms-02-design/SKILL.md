...
> **Model tier.** This stage is product/architecture judgement. Use the current strong model, with high/xhigh for auth/RLS/payments/PII, lifecycle, migrations, queues, or cross-module contracts. Spark (`gpt-5.3-codex-spark`) may be used only as a read-only explorer for a compact evidence map; the lead must verify citations before turning them into the design contract.
