...
4. For each wave record: scope, files, acceptance criteria, implementation profile, risk triggers, required self-check roles, validation, and expected 04b depth.
...
- **Profile E — Evidence-assisted:** use when code discovery is broad or uncertain. Spark/explorer may be used for read-only evidence lookup only; final decisions stay with the main agent.
